...
code += `\n;globalThis.__T={state,validateRuntime,resolveItems,renderHome,renderSimulation,renderSimulationHub,renderTrainingHub,renderTrainingModule,renderGCATTrainingHub,renderGCATSimulationTraining,renderGCATTopicPicker,allGCATEntries,startGCATFocused,v12PushCurrent,goBack,renderOrientationHub,renderQuickReview,startSession,responseStore,storageId,renderLikert,showTrainingFeedback,renderLeadership,showLeadershipFeedback,profileReportData,leadershipDiagnostics,normalizedDirectional,alignmentScore,LIKERT,CRITERION_AR,handleClick,confidenceAnalysis,filteredAttempts,renderResultsHome};`;
...
ok('GCAT training page is icon based',['تدريب مختلط عشوائي','العددي فقط','اللفظي فقط','التجريدي فقط','تدريب حسب الموضوع','تدريب حسب المحاكاة','أخطائي فقط','المفضلة'].every(x=>gh.includes(x)));
...
T.state.session=null;T.startGCATFocused('numerical');ok('GCAT numerical focused is 14 only',T.state.session.items.length===14&&T.state.session.items.every(x=>x.kind==='numerical'));
T.state.session=null;T.startGCATFocused('verbal');ok('GCAT verbal focused is 14 only',T.state.session.items.length===14&&T.state.session.items.every(x=>x.kind==='verbal'));
T.state.session=null;T.startGCATFocused('abstract');ok('GCAT abstract focused is 14 only',T.state.session.items.length===14&&T.state.session.items.every(x=>x.kind==='abstract'));
